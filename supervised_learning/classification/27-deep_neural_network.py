...
class DeepNeuralNetwork:
    """Class defining a deep neural network for multiclass classification."""
    ...
    def forward_prop(self, X):
        ...
        for i in range(1, self.__L + 1):
            ...
            # Change activation to softmax for multiclass classification
            if i == self.__L:
    ...
    def cost(self, Y, A):
        """
        Calculates the cross-entropy cost of the model.

        Args:
            Y (numpy.ndarray): One-hot encoded labels with shape (classes, m).
        ...
        """
        m = Y.shape[1]
        cost = -np.sum(Y * np.log(A + 1e-8)) / m  # Add a small epsilon to prevent log(0)
        return cost
    ...
    def evaluate(self, X, Y):
        ...
        accuracy = np.sum(predictions == Y_argmax) / Y.shape[1] * 100
        cost = self.cost(Y, A)
