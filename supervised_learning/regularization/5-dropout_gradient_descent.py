...
def dropout_gradient_descent(Y, weights, cache, alpha, keep_prob, L):
    ...
    Args:
        Y (numpy.ndarray): One-hot matrix of shape (classes, m) containing the correct labels for the data.
    ...
    for layer in range(L, 0, -1):
        ...
        if layer > 1:
            dZ_output = np.dot(weights["W{}".format(layer)].T, dZ_output) * (1 - np.power(A_prev, 2))  # tanh derivative
